...
class YY24(base_strategy):
    ...
    def could_order(self, t_i):
        #FINDORDER
        if not 120000000 >= self.time_now >= 100100000:
            return
        num_order_bid1_q = 0
        num_order_ask1_q = 0
        ...
        and self.bid1_p < self.weighted_p:
    ...
    def show_result(self):
        ...
        if len(self.buy_time):
            ...
            plt.scatter([numtotime(x) for x in self.sell_time], self.sell_history, marker='v', s= 144, c="lime")
            plt.scatter([numtotime(x) for x in self.buy_time], self.buy_history, marker='^', s = 144, c="red")
            plt.legend()
        else:
            plt.figure(figsize=(20,10))
            plt.subplot(311)
    ...
    def time_dependent_event(self):
        ...
        if self.time_now>=132000000 and self.exit_clean_position_done==0 :
